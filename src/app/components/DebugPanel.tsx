...
export const DebugPanel = ({ permissionsStore, handleCheckMediaDevices }: {
    ...
    return (
      <div className="bg-gray-800/50 border border-gray-600/30 rounded-lg p-3 mb-4 backdrop-blur-sm">
            ...
            <div>🎤 Audio Permission: {permissionsStore.audioPermission}</div>
            <div>📹 Video Permission: {permissionsStore.videoPermission}</div>
            <div>🔄 Requesting Audio: {permissionsStore.isRequestingAudio ? '✅' : '❌'}</div>
            <div>✅ Can Use Audio: {permissionsStore.canUseAudio ? '✅' : '❌'}</div>
            <div>❓ Needs Audio Permission: {permissionsStore.needsAudioPermission ? '✅' : '❌'}</div>
          </div>
                  <button
          ...
          <button
            onClick={() => {
              ...
              console.log('[Debug] navigator.mediaDevices.getUserMedia:', typeof navigator.mediaDevices?.getUserMedia);
